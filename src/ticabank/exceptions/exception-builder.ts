/**
 * Ticabank Exception Builder
 * Factory to create Ticabank exceptions with proper error bodies
 */
...
import {
  ...
  UnableLeadException,
  LeadSoldException,
  TicabankValidationException,
} from './ticabank-exception.js';

/**
 * Build error body for Ticabank exceptions
 */
function buildErrorBody(
  status: number,
  error: string,
  message: string,
  instance: string = 'ticabank'
): ErrorBody {
  return {
    status,
    type: `/ticabank/errors/${error}`,
    title: message,
    detail: message,
    instance,
  };
 ...
 */
export function createValidationException(message: string): TicabankValidationException {
  return new TicabankValidationException(message, HttpStatusCodes.BAD_REQUEST);
}
