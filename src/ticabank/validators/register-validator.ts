...
import { TICABANK_PARAMS, UUID_REGEX } from '../constants/parameters.js';
...
export class RegisterRequestValidator extends BaseValidator<RegisterRequestBody> {
  protected validate(): void {
    ...
    } else if (amount < TICABANK_PARAMS.MIN_AMOUNT || amount > TICABANK_PARAMS.MAX_AMOUNT) {
    ...
    } else if (term < TICABANK_PARAMS.MIN_INSTALLMENTS || term > TICABANK_PARAMS.MAX_INSTALLMENTS) {
      this.addError('cantidadCuotas', `cantidadCuotas must be between ${TICABANK_PARAMS.MIN_INSTALLMENTS} and ${TICABANK_PARAMS.MAX_INSTALLMENTS}`);
    }
    ...
    } else if (paymentDay < TICABANK_PARAMS.MIN_PAY_DATE || paymentDay > TICABANK_PARAMS.MAX_PAY_DATE) {
