...
describe('Mibanco Offer API (e2e)', () => {
  const baseUrl = TEST_CONFIG.baseUrl;
  const USER_IDC = '034636351'; // IDC for yaperod_mibanco1@test-yape.com.pe
  ...
  describe('Success Scenarios', () => {
    it('should return 200 with offer data for valid user (no personality)', async () => {
      ...
      const response = await request(baseUrl)
        .post('/creditos-yape/sales/customer-offer/v1/lead/consultar')
        .set('Content-Type', 'application/json')
        .set('X-User-Email', 'yaperod_mibanco1@test-yape.com.pe')
        .send({
